...
2. 使用顶部“当前账单”选择器切换不同文件和 Sheet。
3. 在“总览”查看明细合计、原表余欠、付款抵扣、日/月趋势、产品结构和按月份展开的明细。
4. 在“欠款排行”查看产品维度的金额降序排行，点击排行项可以展开该产品的逐笔明细。
5. 点击“导入表格”可以一次选择多个 `.xlsx`、`.xls` 或 `.xlsm` 文件。每个文件的有效 Sheet 都会追加到当前账单列表，空 Sheet 会被忽略。
