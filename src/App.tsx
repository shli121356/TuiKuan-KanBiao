...
export default function App() {
  ...
  const handleOpenExcel = (ledger: DebtLedger) => {
    ...
    void (async () => {
      try {
        const fileResponse = await fetch(ledger.sourceUrl as string);
        if (!fileResponse.ok) throw new Error('无法读取原始文件');
        const saveResponse = await fetch('/__open-file/save', {
          body: await fileResponse.arrayBuffer(),
          headers: {
            'Content-Type': 'application/octet-stream',
            'X-File-Name': encodeURIComponent(ledger.sourceFile),
          },
          method: 'POST',
        });
        if (!saveResponse.ok) throw new Error('本机文件打开服务不可用');
        const { token } = await saveResponse.json() as { token?: string };
        if (!token) throw new Error('本机文件打开服务返回无效结果');
        const launchResponse = await fetch('/__open-file/launch', {
          body: JSON.stringify({ token }),
          headers: { 'Content-Type': 'application/json' },
          method: 'POST',
        });
        if (!launchResponse.ok) throw new Error('无法启动本机 Office/WPS');
        setError('');
      } catch {
        const download = document.createElement('a');
        download.href = ledger.sourceUrl as string;
        download.download = ledger.sourceFile;
        download.click();
        setError('本机 Office/WPS 未能自动启动，原始文件已下载，请用 Excel 或 WPS 打开');
      }
    })();
  };
