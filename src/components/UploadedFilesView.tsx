...
export function groupUploadedLedgers(ledgers: DebtLedger[]): UploadedFolderGroup[] {
  const groups = new Map<string, UploadedFolderGroup>();

  ledgers.forEach((ledger) => {
    const folderName = ledger.sourceFolder || ledger.debtorName;
    const key = `${folderName}::${ledger.sourceFile}`;
  ...
  return [...groups.values()].sort((first, second) => {
    return first.folderName.localeCompare(second.folderName, 'zh-CN')
      || first.companyName.localeCompare(second.companyName, 'zh-CN')
      || first.sourceFile.localeCompare(second.sourceFile, 'zh-CN');
  });
}
    ...
    <div className="content-stack uploaded-files-page">
      <Reveal className="uploaded-files-hero">
        <div>
          ...
          <h1>上传文件</h1>
          <p>按人名、公司和原始文件排列；同名文件保持独立，点击卡片查看其中的表格。</p>
        </div>
        <div className="uploaded-files-summary">
          <div><strong>{groups.length}</strong><span>个文件</span></div>
      ...
      {groups.length === 0 ? (
        <Reveal className="uploaded-files-empty">
          ...
          <h2>还没有上传文件</h2>
          <p>导入 Excel 后，文件卡片会出现在这里。</p>
        </Reveal>
      ) : (
        <Reveal className="uploaded-folder-grid">
          {groups.map((group) => (
            <button
              aria-label={`打开文件 ${group.folderName} ${group.companyName} ${group.sourceFile}`}
            ...
            >
              ...
              <span className="uploaded-folder-card-copy">
                ...
                <small>{group.companyName}</small>
              </span>
              <span className="uploaded-folder-card-meta">{group.sourceFile} · {group.ledgers.length} 张表 · {group.rowCount} 条明细</span>
          ...
          <div aria-labelledby="uploaded-modal-title" aria-modal="true" className="uploaded-modal" onMouseDown={(event) => event.stopPropagation()} role="dialog">
            {selectedLedger ? (
              ...
              <>
                ...
                <p className="uploaded-modal-caption">{openFolder.companyName} · {openFolder.sourceFile} · {openFolder.ledgers.length} 张表 · {openFolder.rowCount} 条明细</p>
